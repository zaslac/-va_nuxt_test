<template>

  <div class="container">

    <div class="section_title">
      <h5 v-html="data.sub_title"></h5>
      <h2 v-html="data.title"></h2>
    </div>

    <div class="single_map_component">

      <iframe frameborder="0" scrolling="no" marginheight="0"
              :src="data.map_iframe"
              width="100%" height="500" style="border:0;" allowfullscreen="" loading="lazy"
              referrerpolicy="no-referrer-when-downgrade"></iframe>

      <div class="map_club_card">
        <div class="club_card_inner">
          <div class="card_title">
            <a :href="data.map_link" target="_blank" ><div class="title" v-if="data.map_item_title" v-html="data.map_item_title"></div></a>
          </div>
          <div class="card_address" v-if="data.map_item_content" v-html="data.map_item_content"></div>
          <div class="card_tel">
            <div class="icon">
              <svg width="16" height="17" viewBox="0 0 16 17" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path
                    d="M13.9996 11.4843V13.2908C14.0003 13.4585 13.9659 13.6245 13.8986 13.7781C13.8313 13.9318 13.7325 14.0697 13.6087 14.1831C13.4849 14.2965 13.3387 14.3828 13.1795 14.4365C13.0203 14.4903 12.8517 14.5102 12.6843 14.4951C10.8277 14.2938 9.04426 13.6606 7.47734 12.6465C6.01953 11.722 4.78356 10.4884 3.8572 9.03351C2.83752 7.46259 2.20294 5.67405 2.0049 3.81277C1.98982 3.64625 2.00965 3.47843 2.06312 3.31998C2.11659 3.16153 2.20254 3.01592 2.31548 2.89244C2.42843 2.76896 2.5659 2.6703 2.71915 2.60274C2.87239 2.53519 3.03805 2.50022 3.20558 2.50006H5.01565C5.30846 2.49718 5.59233 2.60067 5.81435 2.79122C6.03636 2.98178 6.18138 3.24641 6.22236 3.53578C6.29876 4.11389 6.44044 4.68152 6.64471 5.22785C6.72589 5.44338 6.74346 5.67761 6.69534 5.9028C6.64721 6.12799 6.53542 6.33469 6.3732 6.49841L5.60694 7.26315C6.46585 8.77069 7.71655 10.0189 9.22708 10.8761L9.99334 10.1114C10.1574 9.94948 10.3645 9.83791 10.5901 9.78988C10.8158 9.74185 11.0505 9.75939 11.2664 9.8404C11.8138 10.0443 12.3826 10.1857 12.9619 10.2619C13.2549 10.3032 13.5226 10.4505 13.714 10.6759C13.9053 10.9013 14.007 11.189 13.9996 11.4843Z"
                    fill="#D3D3D3"></path>
              </svg>
            </div>
            <a :href="'tel:'+data.map_item_phone_number" v-if="data.map_item_phone_number" v-html="data.map_item_phone_number"></a>
          </div>
          <div class="card_opening_times">
            <div class="status green" v-if="data.map_item_status" v-html="data.map_item_status"></div>
            •
            <div class="time" v-if="data.map_item_closing_time" v-html="data.map_item_closing_time"></div>
          </div>
        </div>
      </div>

    </div>

  </div>

</template>

<script setup>
const props = defineProps({
  data: {
    type: Object,
    required: true,
  },
});
const {isMobile} = useDevice();
</script>
